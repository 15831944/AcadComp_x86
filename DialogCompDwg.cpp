...
//[Taka Debug]   wsprintf(sTmp,_T("After Enable Window FALSE(%d)"),acDocManager->documentCount());
//[Taka Debug]   AfxMessageBox(sTmp,0,0);


	AcApDocument *pDocBefore, *pDocAfter;
	pDocBefore = acDocManager->mdiActiveDocument();	

//[Taka Debug]   wsprintf(sTmp,_T("After Getiing DocBefore (%d)"),acDocManager->documentCount());
//[Taka Debug]   AfxMessageBox(sTmp,0,0);
...
//[Taka Debug]   wsprintf(sTmp,_T("After Open Drawing (%d)"),acDocManager->documentCount());
//[Taka Debug]   AfxMessageBox(sTmp,0,0);
...
//[Taka Debug]   wsprintf(sTmp,_T("After Getiing DocAfter (%d)"),acDocManager->documentCount());
//[Taka Debug]   AfxMessageBox(sTmp,0,0);

	if ( acDocManager->documentCount() != 1){
		acDocManager->sendStringToExecute(pDocBefore, DCD_DUMMY_COMMAND, false, true, false);  //Taka あやしい
	}
